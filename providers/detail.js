...
// Make requests for each provider URL and save response
async function fetchProviderDetails() {
    const providers = await getProviders();
    
    for (const provider of providers) {
        try {
            const url = `https://slotcatalog.com${provider.href}`;
            const response = await fetch(url);
            const html = await response.text();
            
            // Create filename from provider title
            const filename = provider.title.replace(/[^a-zA-Z0-9]/g, '_');
            
            // Save response to file
            const outputPath = path.join(__dirname, 'details', `${filename}.html`);
            
            // Create details directory if it doesn't exist
            if (!fs.existsSync(path.join(__dirname, 'details'))) {
                fs.mkdirSync(path.join(__dirname, 'details'));
            }
            
            fs.writeFileSync(outputPath, html);
            console.log(`Saved details for ${provider.title}`);
            
            // Add delay between requests
            await new Promise(resolve => setTimeout(resolve, 1000));
        } catch (error) {
            console.error(`Error fetching details for ${provider.title}:`, error);
        }
    }
}
